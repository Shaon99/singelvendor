...
class DashboardController extends Controller
{
    public function ecommerce()
    {
        ...
        $data['totalSales'] = Order::where('status',2)->sum('subtotal');
        /*$sum = 0;
        if (count($a) > 0){
            foreach ($a as $row){
        ...
        $net = OrderProduct::join('products','products.id','order_product.product_id')
                            ->join('orders','orders.id','order_product.order_id')
                            ->where('orders.status',2)->get();
        $profit = 0;
        foreach ($net as $n) {
            $profit = $profit + ($n->qty * $n->price) - ($n->qty * $n->buying_price);
        }
