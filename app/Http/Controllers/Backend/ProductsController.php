...
class ProductsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $filePath = public_path('/upload/products_images');
        $input['imagename'] = $filename;
        $img->resize(650,650, function ($const) {
            $const->aspectRatio();
...
//        $request->image = $filename;
        ...
        if($request->hasfile('images'))
        {
           $product_id = product::select('id')->latest('id')->first();
            ...
            foreach($request->file('images') as $image)
            {
                ...
                $filepath2 = public_path('upload/products_images/sub_images');
                $input['subImageName'] = $name;
                $img2->resize(650,650, function ($const) {
                    $const->aspectRatio();
                })->save($filepath2.'/'.$input['subImageName']);
    ...
    public function update(Request $request, product $product)
    {
        ...
        if($request->hasfile('images'))
        {
            ...
            foreach($request->file('images') as $image)
            {
                ...
                $filePath2= public_path('/upload/products_images/sub_images');
                $input['subImage'] = $name;
                $img1->resize(650,650,function ($const){
                    $const->aspectRatio();
                })->save($filePath2.'/'.$input['subImage']);
                $subImages = new SubImage();
    ...
    {
        $product = product::with('sub_images')->with('brand')
            ->with('category')->with('tag')->with('sub_category')->find($request->id);
