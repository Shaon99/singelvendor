...
class CheckoutController extends Controller
{
    public function index(Request $request){
        ...
        $carts=CartShopping::where('user_id', $id)->get();

        if(count($carts)<1){
            return Redirect()->back()->with('success3', 'Cart is Empty Please Add Some product');
    ...
    $notify=$order->order_code;


     Notification::send($admin, new OrderNotification($name, $notify_order));
     

    return view('Frontend.single_pages.order_confirmation',compact('notify')); 
   }
